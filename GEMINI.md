...
- Windows 환경에서 PowerShell 스크립트(`.ps1`)를 실행할 때는 시스템 실행 정책으로 인해 실행이 차단되는 것을 방지하기 위해, 항상 `-ExecutionPolicy Bypass` 옵션을 추가하여 실행해야 합니다. (예: `powershell -ExecutionPolicy Bypass -File .\scripts\build_all_configs.ps1`)
